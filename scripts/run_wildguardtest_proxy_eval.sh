...
case "${MODE}" in
  ...
  run)
    ...
    export RUN_COMMAND_ORIGINAL="MODE=run RUN_ID=${RUN_ID} CPU_THREADS=${CPU_THREADS} WILDGUARD_TRAIN=${WILDGUARD_TRAIN} WILDGUARD_TEST=${WILDGUARD_TEST} bash scripts/run_wildguardtest_proxy_eval.sh"
    write_provenance
