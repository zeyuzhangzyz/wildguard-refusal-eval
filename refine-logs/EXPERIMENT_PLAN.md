...
- Dataset / split: all 1,720 WildGuardTest rows with a response-refusal label;
  train TF-IDF only on WildGuardTrain. A deterministic stratified 80/20
  Train/validation split selects the F1-optimal threshold, rounded to one
  reportable decimal. The final proxy is refit on all Train rows; all 1,720
...
- Threshold protocol: choose the threshold exclusively from a deterministic
  held-out WildGuardTrain validation partition; never use WildGuardTest for
  calibration.
